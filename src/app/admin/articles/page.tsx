      ...
      <main className="flex-1 flex flex-col min-w-0 bg-background/50">
        <header className="h-28 border-b border-border flex items-center justify-between px-12 bg-background/80 backdrop-blur-xl sticky top-0 z-50">
          <div className="space-y-1">
            <h1 className="text-2xl font-black text-foreground tracking-tighter uppercase">Gestion des Articles</h1>
        ...
        <div className="p-12 space-y-8 overflow-y-auto custom-scrollbar">
          {/* Toolbar */}
          <div className="flex flex-col md:flex-row gap-6 items-center justify-between bg-card p-6 rounded-[2rem] border border-border">
            <div className="relative w-full md:w-96">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input className="pl-12 rounded-xl bg-muted border-border text-foreground h-12 focus:ring-primary focus:border-primary transition-all font-medium" placeholder="Rechercher un titre ou un auteur..." />
            </div>
            <div className="flex items-center gap-4 w-full md:w-auto">
              <Button variant="outline" className="flex-1 md:flex-none rounded-xl border-border text-foreground font-black uppercase tracking-widest text-[10px] h-12 px-6 hover:bg-muted">
                <Filter className="mr-2 h-4 w-4" /> Filtres
              </Button>
              <select className="flex-1 md:flex-none h-12 bg-muted border border-border rounded-xl px-4 text-foreground text-[10px] font-black uppercase tracking-widest focus:ring-primary outline-none">
          ...
          <div className="bg-card rounded-[3rem] border border-border overflow-hidden">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  <th className="p-8 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Article</th>
                  <th className="p-8 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Auteur</th>
                  <th className="p-8 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Statut</th>
                  <th className="p-8 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Vues</th>
                  <th className="p-8 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] text-right">Actions</th>
                </tr>
              </thead>
                  ...
                  <tr key={article.id} className="group hover:bg-muted/30 transition-colors">
                    <td className="p-8">
                      <div className="flex items-center gap-6">
                        <div className="h-16 w-16 rounded-2xl bg-muted overflow-hidden border border-border flex-shrink-0">
                        ...
                        <div className="space-y-1">
                          <p className="font-black text-foreground text-lg tracking-tight group-hover:text-primary transition-colors line-clamp-1">{article.title}</p>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{article.category}</span>
                    ...
                    <td className="p-8">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-foreground/80">{article.author}</span>
                      </div>
                    </td>
                    <td className="p-8">
                      <Badge className={`
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-56 bg-card border-border rounded-2xl p-2 shadow-2xl">
                            ...
                            <DropdownMenuItem className="rounded-xl focus:bg-destructive/10 focus:text-destructive p-3 cursor-pointer group">
                              ...
                              <span className="text-xs font-black uppercase tracking-widest text-destructive">Supprimer</span>
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
            ...
            <div className="p-8 border-t border-border flex items-center justify-between bg-muted/20">
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Affichage 1-5 sur 5 articles</p>
